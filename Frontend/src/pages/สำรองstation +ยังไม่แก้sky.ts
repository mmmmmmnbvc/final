...
useEffect(() => {
  ...
  const timer = setInterval(() => {
    setEpochIndex((prev) => {
      ...
      if (next >= uploadedData.raw!.length) {
        return 0;
      }
...
}, [epochIndex, uploadedData.groups, visiblePanels, skyConstellation, metricSatellite, metricDataQuality]);
...
const drawPolar = (
  ...
  allSatellites.sort().forEach((SV: string, idx: number) => {
    ...
    traces.push({
      ...
      hovertemplate:
        `SV: ${SV}<br>Avg SNR: ${avgSNR.toFixed(2)} dB-Hz<extra></extra>`
    } as Plotly.Data);
    });
